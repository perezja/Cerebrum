...
def parse_args():
    ...
    parser.add_argument('chunk_dir', help='Directory with study-specific gene chunk directories.')
    parser.add_argument('prefix', help='')
    parser.add_argument('-c', '--chunksize', type=int, default=3, help='Number of genes to merge per blade on MGI.')
...
def merge_chunks(chunk_dir, prefix, outdir):
    """ process to concatenate all finished chunk output files."""
    
    fn = os.path.join(outdir, prefix + '.txt')

    # find: full path to all files in specified directory
    # tail: cat all but header line
    cmd = "find {} -maxdepth 1 -type f | xargs cat >> {}".format(chunk_dir, fn) 

    subprocess.check_call(bsub(cmd, 'merge'), shell=True)


def get_gene_list(chunk_list, chunk_name, outdir, process_container, process_dict, log_fp):

    exc = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'find_list_from_chunks.py')
    cmd = " ".join([exc, chunk_list, chunk_name, '-o', outdir])

    with open(log_fp, 'w+') as fp:
        po = subprocess.Popen(bsub(cmd, 'bitcoin_mining'), stdout=fp, stderr=subprocess.STDOUT, shell=True)
...
def main(args):
    ...
    chunk_outdir = tempfile.mkdtemp(dir=tmpdir)
    ...
    for k,(chunk_name, chunk_list) in enumerate(loop_chunks(chunk_path_list, args.chunksize, chunk_dict)):
        ...
        get_gene_list(gl_path.name, chunk_name, chunk_outdir, chunk_processes, chunk_dict, log_outfile)
    ...
    merge_chunks(chunk_outdir, args.prefix, args.output_dir) 
